...
with open('/home/pgajo/working/food/src/translate/glossary_es.csv', 'r',  encoding='utf-8') as csv_file:
...
for recipe in pbar:
    ...
    entity_translations = translator.translate_text([recipe[f'text_{src_lang}'][entity[0]:entity[1]] for entity in recipe[f'ents_{src_lang}']], 
                                                source_lang = src_lang,
                                                target_lang = tgt_lang,
    ...
    for i, entity in enumerate(recipe[f'ents_{src_lang}']):
        ...
        recipe[f'text_{tgt_lang}'] += translated_text
...
save_path = os.path.join(os.path.dirname(json_path), f'EW-TT-MT_{src_lang}-{tgt_lang}3.json')
