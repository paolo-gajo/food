...
# def shuffle_aligned_entities(sample):
#     print(sample)
#     # Extract texts and entities for both languages
#     text_en = sample['text_en']
#     entities_en = sample['entities_en']
#     text_it = sample['text_it']
#     entities_it = sample['entities_it']

#     # Split text into segments based on entities for both languages
#     def split_text(text, entities):
#         segments = []
#         prev_end = 0
#         for start, end, label in entities:
#             if start > prev_end:
#                 segments.append((text[prev_end:start], None))  # Non-entity text
#             segments.append((text[start:end], label))
#             prev_end = end
#         if prev_end < len(text):
#             segments.append((text[prev_end:], None))  # Trailing non-entity text
#         return segments

#     segments_en = split_text(text_en, entities_en)
#     segments_it = split_text(text_it, entities_it)

#     # Shuffle segments while keeping the entity labels
#     entity_indices = [i for i, seg in enumerate(segments_en) if seg[1] is not None]
#     random.shuffle(entity_indices)

#     # Reassemble the texts and update entities
#     def reassemble_text(segments, entity_indices):
...
#         for seg in segments:
#             if seg[1] is None:
#                 new_text += seg[0]  # Add non-entity text as is
#                 current_pos += len(seg[0])
#             else:
#                 entity_seg = segments[entity_indices[entity_counter]]
#                 entity_counter += 1
#                 new_text += entity_seg[0]
#                 new_start = current_pos
#                 new_end = new_start + len(entity_seg[0])
#                 new_entities.append([new_start, new_end, entity_seg[1]])
#                 current_pos = new_end
#         return new_text, new_entities

#     new_text_en, new_entities_en = reassemble_text(segments_en, entity_indices)
#     new_text_it, new_entities_it = reassemble_text(segments_it, entity_indices)

#     return {'text_en': new_text_en, 'entities_en': new_entities_en, 'text_it': new_text_it, 'entities_it': new_entities_it}
...
example_context = "4 maturo noci di cocco;1 tazza latte evaporato;1 tazza gin;3 cucchiai zucchero (1 cucchiaino terra cannella;1/2 cucchiaino grattugiato fresco noce moscata"
example_query = "ripe"
input = tokenizer(example_query, example_context, return_tensors="pt")
...
import pandas as pd
example_df = pd.DataFrame(example, columns=['token_pos', 'token_id', 'token_str'])
